...
{  
  ...
  return (
    <div className="element">
      ...
      <button type="button" className={`button element__delete ${!isOwn ? `element__delete_invisible` : ``} `} onClick={handleDeleteClick} aria-label="Удалить место" />
      <div className="element__caption">
        <h3 className="block element__text">{card.name}</h3>
        <div className="like">
          <button type="button" className={`like__button ${isLiked && 'like__button_checked'}`} onClick={handleCardLike} aria-label="Поставить лайк" />
